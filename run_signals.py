...
from pipeline.indicators.trend import calculate_ema, calculate_macd
from pipeline.indicators.momentum import calculate_rsi, calculate_stochastic
...
def load_ai_model() -> Any:
    ...
    try:
        ...
        model.policy.eval()
        logging.info(f"AI model loaded from {weights_path} on {device}")
        return model, device
    except Exception as e:
        logging.error(f"Error loading model weights: {e}")
        return None, 'cpu'

def preprocess_for_inference(df: pd.DataFrame, scaler: Any, device: str, window_size: int = 60) -> torch.Tensor | None:
    ...
    tensor = torch.FloatTensor(scaled_data).unsqueeze(0).to(device)
    return tensor

def process_symbol(symbol: str, timeframe: str, model: Any, bot: TelegramBot, scaler: Any, device: str) -> None:
    """
    Fetches data, gets an AI action, parses the signal, and sends it for a single symbol.
    """
    try:
        # Only log when signal found, not every symbol
        ...
        # Add indicators
        df = calculate_ema(df, periods=[20, 50, 200])
        ...
        if df.empty or df['atr'].isnull().all():
            logging.warning(f"Could not calculate indicators for {symbol}. Skipping.")
            return
        ...
        if model and scaler:
            state_tensor = preprocess_for_inference(df, scaler, device)
            if state_tensor is not None:
                with torch.no_grad():
                    action, _ = model.select_action(state_tensor)

        # Random action if no model
        if action == 1 and not model:
...
def scan_job(timeframe: str, model: Any, bot: TelegramBot, symbols: List[str], scaler: Any, device: str) -> None:
    """
    The main job function. It processes symbols concurrently.
    """
    logging.info(f"========== Running {timeframe} Scan Cycle ==========")
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        futures = [executor.submit(process_symbol, symbol, timeframe, model, bot, scaler, device) for symbol in symbols]
...
def main() -> None:
    ...
    scan_job(timeframe='1h', model=ai_model, bot=bot, symbols=symbols_to_scan, scaler=scaler, device=device)

    # Scan every 30 minutes
    schedule.every(30).minutes.do(scan_job, timeframe='1h', model=ai_model, bot=bot, symbols=symbols_to_scan, scaler=scaler, device=device)
    
    # 4H scans at specific times
    for hour in ["00:01", "04:01", "08:01", "12:01", "16:01", "20:01"]:
        schedule.every().day.at(hour).do(scan_job, timeframe='4h', model=ai_model, bot=bot, symbols=symbols_to_scan, scaler=scaler, device=device)
